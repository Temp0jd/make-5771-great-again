...
- 完全移除 v0.4.7 新增的观测台、机械鸮和月蛾原创图片，不再混入额外世界观素材。
- 运行页改用软件原有的《忘却前夜》角色表情作为视觉焦点；顶部、状态、空页面和提示区域继续保留整套既有表情反馈。
- 保留更清晰的深色/亮色配色、金色选中标记和细微背景装饰，不改变任何按钮位置与操作流程。
- 保留 OFL-1.1 授权的 Noto Sans CJK SC UI 字形子集，Windows 未安装中文语言包时也能显示内置中文。
- 本次不改变识别、运行、流程数据与 `.m5771pack` 格式。
