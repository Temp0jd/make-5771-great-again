...
- **模板工具**：全局热键快速截图框选（热键可自定义）、PNG/JPG 拖放导入、局部搜索区域、后台线程识别测试（全图扫描毫秒级）、匹配位置可视化、分辨率不符自动缩放
...
格式详细说明见 [docs/M5771_PACKAGE_SPEC.md](docs/M5771_PACKAGE_SPEC.md)。项目还提供了不含图片的 [Auto 起步包](examples/auto-starter.m5771pack)（导入后可自行绑定截图），以及[活动刷同调示例包](examples/morimens-event-tongdiao.m5771pack)（单步守护循环覆盖「挑战→助战→上场→自动→重新战斗」全流程并处理调查弹窗，含 11 张模板）。
