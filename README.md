...
- **识别引擎**：自适应 RGB 快速候选 + 按需 Top-4/ZNCC 结构复核（区分同款按钮文字）+ 有效像素/透明 Alpha 掩码；可选软 ROI 越界恢复，WaitAny 错峰扫描并在点击前全分支复核
- **性能控制**：低占用 / 均衡 / 极速三档（2 / 4 / 8 线程），无候选时自动降低扫描频率，发现候选后临时提速；全程纯 Rust，无 OpenCV、AI 模型或 GPU 运行时
- **ROI 安全**：新流程可启用智能越界恢复；缺少该配置的旧流程默认保持硬 ROI，避免点击原搜索区域外的相似控件
